...
void loop()
{
  ...
  if (Firebase.ready() && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0)){
    ...
    //Serial.printf("Get BookingID... %s\n", Firebase.getString(fbdo, F("/keyboxes/dkgC3kfhLpkKBysY_C-9/accessingBooking/accessExpired")) ? String(fbdo.to<float>()).c_str() : fbdo.errorReason().c_str());
     Serial.printf("Get Number... %s\n", Firebase.getFloat(fbdo, F("/keyboxes/dkgC3kfhLpkKBysY_C-9/accessingBooking/number")) ? String(fbdo.to<float>()).c_str() : fbdo.errorReason().c_str());
     Serial.printf("Get Requested... %s\n", Firebase.getString(fbdo, F("/keyboxes/dkgC3kfhLpkKBysY_C-9/accessingBooking/accessRequested")) ? String(fbdo.to<String>()).c_str() : fbdo.errorReason().c_str());
     Serial.printf("Get Expired... %s\n", Firebase.getString(fbdo, F("/keyboxes/dkgC3kfhLpkKBysY_C-9/accessingBooking/accessExpired")) ? String(fbdo.to<String>()).c_str() : fbdo.errorReason().c_str());
     
      Serial.println();
  }
/*
  if (Firebase.ready() && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0) && false)
  {
    sendDataPrevMillis = millis();
    ...
    //The function, fbdo.dataType() returns types String e.g. 
    //string, boolean, int, float, double, json, array, blob, file and null.
