    ...
    return (
        ...
        >
            <Stack justifyContent="center">
                <Typography variant="h6" sx={{whiteSpace: "nowrap"}}>{item.name}</Typography>
            </Stack>
        </Paper>
    )
}
    ...
    return (
        <Stack direction="row" spacing={2} justifyContent={"space-between"}>
            <Stack direction="row" spacing={2}>
                ...
                {
                    selectedBooking.message.length > 0 ?
                        <Chip
                            label={"Message: " + selectedBooking.message.substr(0, 100) + (selectedBooking.message.length > 100 ? "..." : "")}
                            icon={<CommentIcon/>}/>
                        : null
                }
            </Stack>
